...
def lingual(img, type, ori):
    ...
    ret2, thresh_img = cv2.threshold(gray_img, np.min(gray_img), np.max(gray_img), cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    ...
    cv2.putText(org, "6", (cervix_pointr[0] + 5, cervix_pointr[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255),
                2)
    cv2.putText(org, "2", (subTop_tooth[0] + 5, subTop_tooth[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255),
                2)
