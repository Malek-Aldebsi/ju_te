...
#     shape_match = cv2.matchShapes(blank_image1, blank_image2, cv2.CONTOURS_MATCH_I2, 0)
...
def distal(img):
    org = img
    perfect = cv2.imread(os.path.join(settings.STATIC_ROOT, "engine/distal-perfect.jpg"), cv2.IMREAD_UNCHANGED)
    gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ret, thresh_img = cv2.threshold(gray_img, 40, 255, 0)
    gcnt, mask = find_rubber(img)
    thresh_img = cv2.bitwise_and(thresh_img, thresh_img, mask=mask)
